...
int RMDServoState::requestPosition(long int deg_100, bool wait)
{
    unsigned char msg[8] = {
        0xA4, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00
    };
    int32_t angleControl = deg_100;
    int32_t maxSpeed = 5;
    msg[2] = *(uint8_t *)(&maxSpeed);
    msg[3] = *((uint8_t *)(&maxSpeed)+1);
    msg[4] = *(uint8_t *)(&angleControl);
    msg[5] = *((uint8_t *)(&angleControl)+1);
    msg[6] = *((uint8_t *)(&angleControl)+2);
    msg[7] = *((uint8_t *)(&angleControl)+3);
    if ( !(CAN_OK == m_pcan_master->sendMsgBuf(device_id, 0, 8, msg)) ) { 
        return 0; 
    } else {
        return wait ? waitForFrame() : 1;
